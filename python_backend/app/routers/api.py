...
def download_brd(reportId: str):
    try:
        # reportId can be a URL or a simple name, depending on what the frontend passes
        html_content = brd_service.generate_brd_html(reportId)
...
async def download_api_test_cases(repo_url: str, refresh: bool = False, tool: str = None):
    import asyncio
    try:
        ...
        html_file = await asyncio.to_thread(
            api_test_case_service.generate_api_test_cases, repo_url, None, None, _force, selected_tool=tool
        )
...
async def download_ui_test_cases(projectId: str, refresh: bool = False, tool: str = None):
    import asyncio
    try:
        # Detect stale/empty cache and force refresh automatically
        ...
        html_file = await asyncio.to_thread(
            ui_test_case_service.generate_ui_test_cases, projectId, None, None, _force, selected_tool=tool
        )
        if not html_file or not Path(html_file).exists():
            from fastapi import HTTPException
            raise HTTPException(status_code=500, detail="Report generation failed — HTML file was not produced.")
